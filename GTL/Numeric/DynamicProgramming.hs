module GTL.Numeric.DynamicProgramming (optimalStrategy, gibbsSmoothedOptimalStrategy) where
...
gibbsSmoothedOptimalStrategy mdp eps x = fromList $ toList $ mapVector (\y -> exp $ y / eps) $ qF $ indexF x
    where (Result _ qF) = solve $ setup mdp
